...
def add_signal_to_paper(db: Session, signal: Signal) -> PaperTrade:
    ...
    size_usd = min(signal.suggested_size_usd or DEFAULT_SIZE_USD, STARTING_BALANCE * 0.05)
...
def close_from_signal(db: Session, signal: Signal) -> int:
    ...
    trades = (
        db.query(PaperTrade)
        .filter(PaperTrade.symbol == signal.symbol, PaperTrade.status == "open")
        .all()
    )
    for trade in trades:
...
def paper_account_summary(db: Session) -> Dict:
    ...
    total_pnl = sum(trade.pnl for trade in trades if trade.status != "open")
    open_unrealized = sum(_calculate_pnl(trade, trade.exit_price or trade.entry_price) for trade in trades if trade.status == "open")
    ...
    return {
        ...
        "current_equity": round(STARTING_BALANCE + total_pnl + open_unrealized, 6),
        "today_pnl": round(today_pnl, 6),
        "total_pnl": round(total_pnl + open_unrealized, 6),
...
def _calculate_raw_pnl(trade: PaperTrade, exit_price: float) -> float:
    ...
    return round(trade.size_usd * trade.leverage * change * direction, 6)
